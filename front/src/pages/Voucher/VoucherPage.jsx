...
import { buscarEstudianteVoucherService, buscarVoucherService, crearVoucherService, obtenerVouchersService } from "../../api/voucherService";
...
const VoucherPage = () => {
  ...
  const guardarCarrera = async (params) => {
    ...
    if(!resp.data) { message.error('Ocurrio un error'); return  }
    if(resp.data.ok) {
          ...
          <Form layout="vertical" form={formVoucher} onFinish={guardarCarrera}>
            <div className="vacantesPageContainerFormCrearVacante">
              ...
              <Form.Item label="DNI" name="DNI" rules={[{ required: true,  }]}>
                <Input placeholder="Ingresa el codigo del voucher" onChange={buscarEstudiante} maxLength={8} />
              </Form.Item>
              <Form.Item label="Codigo" name="CODIGO" rules={[{ required: true,  }]}>
                <Input placeholder="Ingresa el codigo del voucher" maxLength={10} />
              </Form.Item>
              <Form.Item label="Fecha" name="FECHA_PAGO" rules={[{ required: true,  }]}>
                <DatePicker placeholder="Fecha del voucher" style={{ width: '100%' }} />
              </Form.Item>
